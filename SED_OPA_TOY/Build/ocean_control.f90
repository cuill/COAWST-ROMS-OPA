...
!
        CALL inp_par (iNLM)
        IF (FoundError(exit_flag, NoError, 124,                         &
     &                 "ROMS/Drivers/nl_ocean.h")) RETURN
!        CALL mp_barrier(ng,iNLM)  ! DDMITRY
!
...
!$OMP PARALLEL
          DO thread=0,numthreads-1
            CALL wclock_on (ng, iNLM, 0, 159, "ROMS/Drivers/nl_ocean.h")
          END DO
!$OMP END PARALLEL
        END DO
!
...
!$OMP PARALLEL
      CALL initial
!$OMP END PARALLEL
      IF (FoundError(exit_flag, NoError, 230,                           &
     &               "ROMS/Drivers/nl_ocean.h")) RETURN
!
...
!$OMP END PARALLEL
      IF (FoundError(exit_flag, NoError, 350,                           &
     &               "ROMS/Drivers/nl_ocean.h")) RETURN
!
...
!$OMP PARALLEL
        DO thread=0,numthreads-1
          CALL wclock_off (ng, iNLM, 0, 453, "ROMS/Drivers/nl_ocean.h")
        END DO
!$OMP END PARALLEL
      END DO
!
