...
export   COAWST_APPLICATION=CB_OPA_TOY
...
#export   MY_ROOT_DIR=/ches/data10/VIMSMPC/lcui/CSOMIO/trunk_oil_new/COAWST-ROMS-OIL
#export   MY_ROOT_DIR=/ches/data10/VIMSMPC/lcui/CSOMIO/trunk_1d
export   MY_ROOT_DIR=/ches/data10/VIMSMPC/lcui/CSOMIO/trunk_oil_eulr
  ...
  case "$FORT" in
    ifort )
      if [ "${which_MPI}" = "mpich" ]; then
        ...
        export PATH=/opt/hpc/intel/openmpi/bin:$PATH
      fi
      ;;
    ...
    ifort )
      ...
      if [ -n "${USE_MPI:+1}" ]; then
        if [ "${which_MPI}" = "mpich" ]; then
          ...
          export      MCT_INCDIR=/gpfs/research/coaps/common/opt/hpc/intel/openmpi/include/mct28
          export      MCT_LIBDIR=/gpfs/research/coaps/common/opt/hpc/intel/openmpi/lib64/mct28
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            ...
            export NETCDF_INCDIR=/opt/hpc/intel/openmpi/include
          fi
        else
        ...
        export     NETCDF_INCDIR=/opt/hpc/intel/openmpi/include
        export     NETCDF_LIBDIR=/opt/hpc/intel/openmpi/lib64
      fi
      ;;
